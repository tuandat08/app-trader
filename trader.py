...
def run(cfg, should_stop=None):
    ...
    data_ex = make_exchange()        # DỮ LIỆU thị trường thật (công khai, không key) → tín hiệu
    ...
    while True:
        try:
            ...
            if cfg.max_capital and cfg.max_capital > 0:
                peak = min(peak, budget)   # bỏ đỉnh cũ sai thang (từ lần chạy vốn khác)
            ...
            for sym in list(state["open"].keys()):
                ...
                if reason:
                    ...
                    emoji = "✅" if pnl > 0 else "🔴"
                    kq = "LỜI" if pnl > 0 else "LỖ"
                    # Tổng tích luỹ từ nhật ký (để báo cả bức tranh)
                    try:
                        ...
                        tong = ""
                    note.event(
                        f"{emoji} ĐÓNG {sym} — {kq} (${pnl:+.2f})\n"
                        f"• Lý do: {reason}\n"
                        f"• Giá ra: ${exit_px:.6f} ({pnl_pct:+.2f}%) · {r_mult:+.2f}R\n"
                        f"• Giữ: {held_bars}h" + tong)
                    del state["open"][sym]
                    _save_state(state)
                else:
