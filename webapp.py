...
ENV_KEYS = [
    "TRADE_MODE", "DRY_RUN", "SYMBOLS", "USE_GAINERS", "INITIAL_CAPITAL", "MAX_CAPITAL",
    "RISK_PER_TRADE", "TP_PCT", "MAX_SL_PCT", "MAX_OPEN_TRADES", "DAILY_STOP",
...
def api_live_stats():
    try:
        ...
        cfg = config_mod.Config()
        return jsonify(live_stats(capital=cfg.initial_capital))
    except Exception as e:
...
def api_scan_trades():
    ...
    try:
        ...
        return jsonify({"ok": True, "trades": trades, "summary": summary, "analysis": analysis})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 200
